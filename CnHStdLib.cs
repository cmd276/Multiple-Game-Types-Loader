...
## -------------------------------------------------------------------- inMission
function cnh::inMissionStart()
{
    $server::PointLimit = 50;
    cnh::onMissionLoad();
    for(%i = 0; %i < %players; %i++)
    {
        // get this players ID.
        ...
        {
            cnh::player::onAdd(%player);
            // lets act like they jsut spawned.
            cnh::vehicle::onAdd(%vehicle);
        }
    }
}
function cnh::inMissionEnd()
{
}

## -------------------------------------------------------------------- Player
function cnh::player::onAdd(%player)
{
    %player.seizures = 0;
    //say(%player, 0, "Bugfix testing for Capture and Hold.  Please report problems on the Editing and Scripting forum");
}   
## -------------------------------------------------------------------- Vehicle
function cnh::vehicle::onAdd(%vehicleId)
{
...
function cnh::vehicle::OnDestroyed( %victim, %destroyer )
{
    %victimNum = playerManager::vehicleIdToPlayerNum( %victim );
    %destroyerNum = playerManager::vehicleIdToPlayerNum( %destroyer );

    if(getHUDName(%destroyer) == "Blue Turret") 
        $blueKills++;
    if(getHUDName(%destroyer) == "Purple Turret") 
        $purpleKills++;
    if(getHUDName(%destroyer) == "Red Turret") 
        $redKills++;
    if(getHUDName(%destroyer) == "Yellow Turret") 
        $yellowKills++;

    if(%destroyerNum == 0) 
        return;
    if(%destroyerNum.color == "BLUE") 
        $blueKills++; // $scores[getTeam(%destoyerNum)]++;
    if(%destroyerNum.color == "PURPLE") 
        $purpleKills++;
    if(%destroyerNum.color == "RED") 
        $redKills++;
    if(%destroyerNum.color == "YELLOW") 
        $yellowKills++;

    if(%victimNum.color == "BLUE") 
        $blueDeaths++;  // $scores[getTeam(%victimNum)]++;
    if(%victimNum.color == "PURPLE") 
        $purpleDeaths++;
    if(%victimNum.color == "RED") 
        $redDeaths++;
    if(%victimNum.color == "YELLOW") 
        $yellowDeaths++;

    if(getTeamScore(1) >= $server::PointLimit) 
        missionWin(1);
    if(getTeamScore(2) >= $server::PointLimit) 
        missionWin(2);
    if(getTeamScore(4) >= $server::PointLimit) 
        missionWin(4);
    if(getTeamScore(8) >= $server::PointLimit) 
        missionWin(8);
}
## -------------------------------------------------------------------- Trigger
